...
export default () => {
  ...
  const exportImage = (domRef: HTMLElement, format: string, quality: number, ignoreWebfont = true) => {
    ...
    setTimeout(() => {
      ...
      toImage(domRef, config).then(dataUrl => {
        ...
        exporting.value = false
        message.error('Export images failure')
      })
    }, 200)
  }
  ...
  // Format the color value of transparency + HexString, used for pptxgenjs
  ...
  // The HTML string formatted into pptxgenjs the required format
  // Core ideas: the HTML string according to the style shard tile, each need to inherit the ancestor element's style information, meet block-level elements need a newline
  ...
  // The SVG path information formatted as pptxgenjs required format
  ...
  // To get the shadow configuration
          ...
          for (let i = 0; i < el.data.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
              ...
              const cellOptions: pptxgen.TableCellProps = {
                ...
                valign: 'middle',
                fontFace: cell.style?.fontname || 'Microsoft jas black',
