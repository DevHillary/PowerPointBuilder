...
// Rich text various interactive event listeners:
// Focusing on the cancellation global shortcuts
// Enter text to synchronize data vuex
// Click the mouse and keyboard state synchronously with rich text to the toolbar
...
// The rich text content synchronization to the DOM
...
// Open the/Close the editor of the edit mode
watch(() => props.editable, () => {
  editorView.setProps({ editable: () => props.editable })
})

// exposed focus methods
const focus = () => editorView.focus()
defineExpose({ focus })

// The rich text command (can be one or more)
// Part of the command to determine whether the current district before the execution is empty, if the selection is empty for selection first
...
// When the mouse is raised, brush format command execution
const handleMouseup = () => {
...
// Initialization and unload Prosemirror editor
onMounted(() => {
