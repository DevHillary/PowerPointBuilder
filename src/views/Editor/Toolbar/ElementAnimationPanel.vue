<template>
  <div class="element-animation-panel">
    ...
    <div class="tip" v-else><IconClick style="margin-right: 5px;" /> The selected elements on the canvas to add animation</div>
        ...
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]">
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              <div style="flex: 3;">The last time：</div>
              <InputNumber 
                :min="500"
                :max="3000"
                :step="500"
            ...
            <div class="config-item">
              <div style="flex: 3;">triggered：</div>
              <Select
                :value="element.trigger"
                @change="value => updateElementAnimationTrigger(element.id, value as 'click' | 'meantime' | 'auto')"
                style="flex: 5;"
              >
                <SelectOption value="click">Take the initiative to trigger</SelectOption>
                <SelectOption value="meantime">With the animation at the same time one</SelectOption>
                <SelectOption value="auto">After an animation</SelectOption>
              </Select>
            </div>
            <div class="config-item">
              <Button style="flex: 1;" @click="openAnimationPool(element.id)">Replace the animation</Button>
            </div>
          </div>
        </div>
      </template>
...
const tabs: TabItem[] = [
  { key: 'in', label: 'The entrance' },
  { key: 'out', label: 'exit' },
  { key: 'attention', label: 'Emphasis on' },
]
...
// The currently selected element into animation information
...
// Drag the synchronous data after the modification of the animation sequence
...
// Modify the element animation duration
...
// Modify the element animation, and perform a preview
...
// Add elements to animation, and perform a preview
...
// Animation on panel select 600 ms and then remove the mask layer, or open the panel into the mouse after the preview will lead to shake
