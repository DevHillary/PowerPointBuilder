<template>
  <div class="chart-style-panel">
    <Button class="full-width-btn" @click="chartDataEditorVisible = true">
      <IconEdit class="btn-icon" /> Edit the chart data
    </Button>
    ...
    <template v-if="handleChartElement.chartType === 'line'">
      <div class="row">
        ...
        <Checkbox 
          ...
          :checked="!showLine" 
          style="flex: 1;"
        >A scatter diagram style</Checkbox>
      </div>
      <div class="row">
        <Checkbox 
          @change="e => updateOptions({ lineSmooth: e.target.checked })" 
          :checked="lineSmooth"
        >Using a smooth curve</Checkbox>
      </div>
    </template>
    ...
    <div class="row" v-if="handleChartElement.chartType === 'pie'">
      <Checkbox 
        ...
        :checked="donut"
      >Circular diagram style</Checkbox>
    </div>
    ...
    <div class="row">
      ...
      <Select style="flex: 3;" :value="legend" @change="value => updateLegend(value as '' | 'top' | 'bottom')">
        <SelectOption value="">Don't show</SelectOption>
        <SelectOption value="top">Is shown in the above</SelectOption>
        <SelectOption value="bottom">Is shown in the below</SelectOption>
      </Select>
    </div>
    ...
    <ButtonGroup class="row">
      <Popover trigger="click" v-model:visible="presetThemesVisible">
        ...
        <Button class="no-padding" style="flex: 2;">Recommended topics</Button>
      </Popover>
      <Button 
        class="no-padding" 
        ...
        @click="addThemeColor()"
      >
        <IconPlus class="btn-icon" /> Add the theme color
      </Button>
    </ButtonGroup>
...
// Set other options: histogram area charts, bar charts, line charts turns round of scatter plot, the pie chart line chart ring figure, line chart switch smooth curve
