...
// The border line of the selection
...
// The current zoom point of the selection
...
// Initialize the data in the chart, the data format and fill to the DOM
const initData = () => {
  const _data: string[][] = []
...
// Shortcuts to monitor: enter move the focus to the next line
const moveNextRow = () => {
...
// Gets the current chart data in the DOM, after finishing format
const getTableData = () => {
  ...
  // The first behavior series, the first listed as project name, the actual data from the second line of the second column
  ...
  for (let colIndex = 1; colIndex < col; colIndex++) {
    let legendsItem = `A series of${colIndex}`
...
// Custom paste events (try to read the table data in the clipboard)
...
// Mouse drag and drop to modify the selected data
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // At the end of the drag and drop, more than half grid automatically expand to the next case (such as drag to the position of more than one and a half, will be automatically extended to two, namely all the same)
