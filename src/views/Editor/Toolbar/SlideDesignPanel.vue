<template>
  <div class="slide-design-panel">
    ...
    <div class="row">
      ...
      >
        <SelectOption value="solid">Pure color fill</SelectOption>
        <SelectOption value="image">Picture fills</SelectOption>
        <SelectOption value="gradient">The gradient</SelectOption>
      </Select>
      ...
      >
        <SelectOption value="contain">The zoom</SelectOption>
        <SelectOption value="repeat">collage</SelectOption>
        <SelectOption value="cover">Zoom covered</SelectOption>
      </Select>
    ...
    <div class="background-gradient-wrapper" v-if="background.type === 'gradient'">
      <div class="row">
        <div style="flex: 2;">Starting point for color：</div>
      ...
      <div class="row">
        <div style="flex: 2;">The finish color：</div>
        <Popover trigger="click">
          <template #content>
            <ColorPicker
      ...
      <div class="row" v-if="background.gradientType === 'linear'">
        <div style="flex: 2;">The gradient Angle：</div>
        <Slider
          class="slider"
          :min="0"
          :max="360"
    ...
    <div class="row"><Button style="flex: 1;" @click="applyBackgroundAllSlide()">Background to the entire application</Button></div>

    <Divider />

    <div class="row">
      <div style="flex: 2;">The canvas size：</div>
      <Select style="flex: 3;" :value="viewportRatio" @change="value => updateViewportRatio(value as number)">
        ...
        <SelectOption :value="0.70710678">The paper A3 / A4</SelectOption>
      </Select>
    </div>
    ...
    <div class="row">
      <div style="flex: 2;">The font color：</div>
      <Popover trigger="click">
        <template #content>
          <ColorPicker
    ...
    <div class="row">
      <div style="flex: 2;">The background color：</div>
      <Popover trigger="click">
        <template #content>
          <ColorPicker
    ...
    <div class="row">
      <div style="flex: 2;">The theme color：</div>
      <Popover trigger="click">
        <template #content>
          <ColorPicker
    ...
    <div class="row"><Button style="flex: 1;" @click="applyThemeToAllSlides()">Application subject to the full</Button></div>
    ...
    <div class="theme-list">
      ...
      >
        <div class="theme-item-content">
          <div class="text" :style="{ color: item.fontColor }">The text Aa</div>
          <div class="colors">
          ...
          <div class="btns">
            ...
            <div class="btn" @click="applyPresetThemeToAllSlides(item)">Application of global</div>
          </div>
        </div>
      </div>
    </div>
...
// Set the background pattern: solid color, picture, gradient
...
// Application of the current page background to the entire page
...
// Set the canvas size (wide high percentage)
