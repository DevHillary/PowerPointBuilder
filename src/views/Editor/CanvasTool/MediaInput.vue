<template>
  <div class="media-input">
    ...
    <template v-if="type === 'audio'">
      <Input v-model:value="audioSrc" placeholder="Please input audio address，e.g. https://xxx.mp3"></Input>
      <div class="btns">
        <Button @click="emit('close')" style="margin-right: 10px;">cancel</Button>
...
const insertVideo = () => {
  if (!videoSrc.value) return message.error('Please input the correct video address')
  emit('insertVideo', videoSrc.value)
}

const insertAudio = () => {
  if (!audioSrc.value) return message.error('Please input the correct audio address')
