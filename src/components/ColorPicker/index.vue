...
// When each time you select the default color, need to add the color to the list of most recently used
...
// Open the color straw
// check whether environment support native color straws, use the native straw support, otherwise use custom straws
...
// Primary color straws
const browserEyeDropper = () => {
  message.success('According to the ESC Button to turn off the color straw')
  ...
  eyeDropper.open().then((result: { sRGBHex: string }) => {
    ...
    message.success('Close the color straw')
  })
}

// Custom color straw based on Canvas
const customEyeDropper = () => {
  ...
  toCanvas(targetRef, { filter, fontEmbedCSS: '', width, height, canvasWidth: width, canvasHeight: height, pixelRatio: 1 }).then(canvasRef => {
    ...
    message.error('Failed to initialize the color straw')
    document.body.removeChild(maskRef)
  })
}
</script>
